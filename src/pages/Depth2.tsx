...
import {
  useRegionData,
  type RegionAnalysisData,
} from "../features/chat/hooks/useRegionData";
    ...
    >
      ...
      <div className="flex flex-col gap-[24px]">
        {issues.map((issue, idx) => {
          const sampleCount =
            (issue as any).sample_count || issue.voices.length;
          const displayCount =
            sampleCount > issue.voices.length
              ? sampleCount
              : issue.voices.length;
      ...
      <div className="bg-white rounded-[16px] p-[40px] border border-[#E0E7E9] shadow-[0px_4px_20px_0px_rgba(0,188,212,0.05)]">
        ...
        {!hasJsonData && (
          <div className="mb-[32px]">
            ...
            <p className="text-[14px] text-[#455A64] leading-[170%]">
              {data.region_name} 관련 게시글 {data.total_posts}건 중{" "}
              {totalVoices}건에서 지역 생활과 관련된 의견이 확인되었습니다.{" "}
                          ...
                          <div className="flex flex-col gap-[6px]">
                            {data.negative_issues[0].voices
                              ...
                              .map((voice, idx) => (
                                ...
                                >
                                  <span className="text-[#00BFA5] shrink-0">
                                    •
                                  </span>
          ...
          <div className="pl-[20px]">
            {data.positive_issues && data.positive_issues.length > 0 ? (
              <>
                ...
                <p className="text-[14px] text-[#455A64] leading-[170%] mb-[12px]">
                  {data.positive_issues[0].context ||
                    `일부 게시글에서 ${data.region_name}의 자연환경에 대한 긍정적 평가가 있었습니다.`}
                </p>
      ...
      <div className="bg-white rounded-[16px] p-[40px] border border-[#E0E7E9] shadow-[0px_4px_20px_0px_rgba(0,188,212,0.05)]">
        {hasJsonLimitations ? (
          ...
          <div className="flex flex-col gap-[20px]">
            ...
            {jsonLimitations.limitations &&
              jsonLimitations.limitations.length > 0 && (
                <div className="mb-[20px]">
                  ...
                  <div className="flex flex-col gap-[8px] pl-[20px]">
                    {jsonLimitations.limitations.map((limitation, idx) => (
                      <p
                        key={idx}
                        className="text-[14px] text-[#374151] leading-[170%]"
                      >
                        • {limitation}
                      </p>
                    ))}
          ...
          <div className="flex flex-col gap-[20px]">
            ...
            <div>
              ...
              <div className="flex flex-col gap-[8px] pl-[20px]">
                ...
                <p className="text-[14px] text-[#374151] leading-[170%]">
                  • 주민이 느끼는 '체감'을 전달하는 것이며, 객관적 사실과 다를
                  수 있습니다
                </p>
              </div>
            </div>
            ...
            <div className="mt-[12px] pt-[20px] border-t border-[#E0E7E9]">
              <p className="text-[14px] text-[#374151] leading-[180%] mb-[12px]">
                이 분석은 '이런 의견도 있다'는 참고 자료로만 활용하시고, 실제
                정책 결정 시에는:
              </p>
              <div className="flex flex-col gap-[8px] pl-[20px]">
                <p className="text-[14px] text-[#374151] leading-[170%]">
...
export default function Depth2Report({
  ...
  console.log(
    "[Depth2Report] 컴포넌트 렌더링 시작, selectedRegion:",
    selectedRegion
  );
  ...
  const jsonReport = useMemo(() => {
    console.log(
      `🔍 jsonReport useMemo 실행 - selectedRegion: "${selectedRegion}"`
    );
    ...
    try {
      ...
      if (report) {
        console.log(
          `✅ JSON 리포트 로드 성공: ${selectedRegion} -> ${report.region}`
        );
      } else {
