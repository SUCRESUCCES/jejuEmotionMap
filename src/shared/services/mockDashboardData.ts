...
// JSON 데이터 파일 import
import HallimData from "../../mocks/Hallim.json";
import AewolData from "../../mocks/Aewol.json";
import JejusiData from "../../mocks/jejusi.json";
import GujwaData from "../../mocks/Gujwa.json";
import JocheonData from "../../mocks/Jocheon.json";
import HangyeongData from "../../mocks/Hangyeong.json";
import DaejeongData from "../../mocks/Daejeong.json";
import AndeokData from "../../mocks/Andeok.json";
import JungmunData from "../../mocks/Jungmun.json";
import SeogwipoData from "../../mocks/Seogwipo.json";
import NamwonData from "../../mocks/Namwon.json";
import PyoseonData from "../../mocks/Pyoseon.json";
import SeongsanData from "../../mocks/Seongsan.json";
import {
  getSSIDataByDateRange,
  getTrendData,
  getRiskLevelFromSSI,
} from "../../features/dashboard/constants/ssiMockData";
...
// 지역별 상세 정보 데이터
const REGION_DETAIL_DATA: Record<
  string,
  Omit<RegionDetail, "지역명" | "ssi">
> = {
  구좌읍: {
    변화율: "+5.2%",
    위험도: "위험",
...
function generateChartDataForRegion(
  ...
  const baseValue = baseSSI[region] || 0.45;
...
export const mockDashboardData = {
  ...
  getRegionSSI: (startDate: string, endDate: string): RegionSSI[] => {
    ...
    const result = REGION_SSI_DATA.map((region) => {
      try {
        ...
        const { ssi } = getSSIDataByDateRange(
          region.지역명,
          startDate,
          endDate
        );
  ...
  extractKeywordsAndSummary: (
    ...
    if (data.주요_이슈_분석) {
      ...
      if (
        data.주요_이슈_분석.긍정이슈 &&
        Array.isArray(data.주요_이슈_분석.긍정이슈)
      ) {
      ...
      if (
        data.주요_이슈_분석.부정이슈 &&
        Array.isArray(data.주요_이슈_분석.부정이슈)
      ) {
      ...
      if (
        data.주요_이슈_분석.중립이슈 &&
        Array.isArray(data.주요_이슈_분석.중립이슈)
      ) {
  ...
  getRegionDetail: async (
    ...
    const regionName =
      Object.keys(REGION_NAME_MAP).find(
    ...
    let calculatedSSI = 0.4;
    let change = 0;
    try {
      const { ssi, change: ssiChange } = getSSIDataByDateRange(
        regionName,
        startDate,
        endDate
      );
