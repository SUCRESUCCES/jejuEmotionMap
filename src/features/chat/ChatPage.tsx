...
import { getSSIDataByDateRange, getTrendData, getRiskLevelFromSSI } from '../dashboard/constants/ssiMockData';
// JSON 리포트 데이터 로더
...
export default function Depth2Report({ 
  ...
  const jsonReport = useMemo(() => {
    ...
    console.log(`[ChatPage] jsonReport 계산 시작 - selectedRegion: "${selectedRegion}"`);
    console.log(`[ChatPage] getRegionReport 함수 존재:`, typeof getRegionReport);
    
    try {
      const report = getRegionReport(selectedRegion);
      if (report) {
        console.log(`[ChatPage] ✅ JSON 리포트 로드 성공! 지역: ${report.region}`);
      } else {
        console.warn(`[ChatPage] ⚠️ JSON 리포트를 찾을 수 없음: ${selectedRegion}`);
      }
      return report;
    } catch (error) {
