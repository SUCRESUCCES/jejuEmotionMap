// SSI JSON 파일 import
import 제주시SSI from '../../../data/제주시_ssi.json';
import 한림읍SSI from '../../../data/한림읍_ssi.json';
import 제주도SSI from '../../../data/제주도_ssi.json';
import 애월읍SSI from '../../../data/애월읍_ssi.json';
import 안덕면SSI from '../../../data/안덕면_ssi.json';
import 서귀포시SSI from '../../../data/서귀포시_ssi.json';
import 대정읍SSI from '../../../data/대정읍_ssi.json';
import 구좌읍SSI from '../../../data/구좌읍_ssi.json';
import 성산읍SSI from '../../../data/성산읍_ssi.json';
import 표선면SSI from '../../../data/표선면_ssi.json';
import 한경면SSI from '../../../data/한경면_ssi.json';
import 중문SSI from '../../../data/중문_ssi.json';
import 조천읍SSI from '../../../data/조천읍_ssi.json';
import 남원읍SSI from '../../../data/남원읍_ssi.json';
...
function generateRegionSSIData(
  ...
  for (let i = 0; i <= totalDays; i++) {
    ...
    const randomVariation = (Math.random() - 0.5) * 0.30;
    ssi += randomVariation;
...
const REGION_CONFIG: Record<string, { baseSSI: number; trend: 'increase' | 'decrease' | 'stable' }> = {
  '구좌읍': { baseSSI: 0.480, trend: 'increase' },
  '조천읍': { baseSSI: 0.420, trend: 'stable' },
  '제주시': { baseSSI: 0.380, trend: 'increase' },
  '애월읍': { baseSSI: 0.450, trend: 'increase' },
  '한림읍': { baseSSI: 0.320, trend: 'stable' },
  '한경면': { baseSSI: 0.420, trend: 'increase' },
  '대정읍': { baseSSI: 0.390, trend: 'stable' },
  '안덕면': { baseSSI: 0.360, trend: 'decrease' },
  '중문': { baseSSI: 0.380, trend: 'stable' },
  '서귀포시': { baseSSI: 0.440, trend: 'increase' },
  '남원읍': { baseSSI: 0.340, trend: 'stable' },
  '표선면': { baseSSI: 0.360, trend: 'stable' },
  '성산읍': { baseSSI: 0.370, trend: 'stable' },
  '추자면': { baseSSI: 0.310, trend: 'decrease' },
  '우도면': { baseSSI: 0.310, trend: 'stable' },
  '제주도': { baseSSI: 0.400, trend: 'increase' }, // 제주도 전체 평균
};
...
export function getSSIDataByDateRange(
  ...
  if (filteredData.length === 0) {
    ...
    if (endDate < dataStartDate) {
      ...
      const avgSSI = earlyData.reduce((sum, point) => sum + point.ssi, 0) / earlyData.length;
    ...
    if (startDate > dataEndDate) {
      ...
      const avgSSI = recentData.reduce((sum, point) => sum + point.ssi, 0) / recentData.length;
      return { ssi: avgSSI, change: 0 };
    }
    
    // 범위 내 데이터가 없지만 날짜 범위는 겹치는 경우 전체 평균 사용
    const avgSSI = regionData.reduce((sum, point) => sum + point.ssi, 0) / regionData.length;
    return { ssi: avgSSI, change: 0 };
  }
  ...
  const change = firstSSI > 0 
    ? ((lastSSI - firstSSI) / firstSSI) * 100 
    : 0;
  ...
  if (filteredData.length === 0) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (weekStart < recentData.length) {
        ...
        const avgSSI = weekData.reduce((sum, point) => sum + point.ssi, 0) / weekData.length;
        weeklyData.push(avgSSI);
      } else {
        weeklyData.push(weeklyData[weeklyData.length - 1] || 0.5);
      }
    }
    return weeklyData.length === 8 ? weeklyData : [0.52, 0.54, 0.58, 0.55, 0.59, 0.62, 0.65, 0.68];
  }
